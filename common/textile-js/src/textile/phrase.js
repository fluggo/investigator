...
const reLinkFenced = /^\["([^\n]+?)":((?:\[[a-z0-9]*\]|[^\]])+)\]/;
const reLinkTitle = /\s*\(((?:\([^\(\)]*\)|[^\(\)])+)\)$/;
const reBareLink = /^https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/;
const reHashtag = /^#([A-Za-z][A-Za-z0-9-]*)(?::([^\s]+))?/;
const reWikiLink = /^\[\[([^|\]]+)(?:\|([^\]]+))?\]\]/;
const reFootnote = /^\[(\d+)(!?)\]/;
...
function parsePhrase ( src, options ) {
  ...
  do {
    ...
    // *******
    // Added for Investigator tools: bare link support
    if(boundary && (m = reBareLink.exec(src))) {
      src.advance(m[0]);
      list.add(['a', { href: m[0] }, m[0]]);
      continue;
    }

    // *******
    // Added for Investigator tools: hashtag support
    if(boundary && (m = reHashtag.exec(src))) {
      src.advance(m[0]);

      const attr = options.keepContext ? {context: src.original()} : {};

      if(m[2])
        list.add(['hashtag', attr, ['tag', {href: m[1]}, '#' + m[1]], ':', ['value', m[2]]]);
      else
        list.add(['hashtag', attr, ['tag', {href: m[1]}, '#' + m[1]]]);

      continue;
    }

    // ******
    // Added for Investigator tools: wiki link support
    if(boundary && (m = reWikiLink.exec(src))) {
      src.advance(m[0]);

      const attr = {href: m[1]};

      if(options.keepContext)
        attr.context = src.original();

      list.add(['wiki', attr, m[2] || m[1]]);
